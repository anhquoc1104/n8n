...
let ExecutionRepository = class ExecutionRepository extends typeorm_1.Repository {
    ...
    async findIfSharedUnflatten(executionId, sharedWorkflowIds) {
        return await this.findSingleExecution(executionId, {
            where: {
    ...
    toQueryBuilder(query) {
        const { accessibleWorkflowIds, status, finished, workflowId, startedBefore, startedAfter, metadata, annotationTags, vote, projectId, workflowVersionId, } = query;
