...
const utils_1 = require("./request-helpers/utils");
function validateUrl(url) {
    if (!url)
        return false;
    return (0, utils_1.tryParseUrl)(url) !== null;
}
...
const getBeforeRedirectFn = (agentOptions, axiosConfig, proxyConfig, sendCredentialsOnCrossOriginRedirect, ssrfBridge) => (redirectedRequest) => {
    ...
    const customProxyUrl = (0, utils_1.getUrlFromProxyConfig)(proxyConfig);
...
async function parseRequestObject(requestObject, ssrfBridge) {
    ...
    (0, utils_1.setAxiosAgents)(axiosConfig, agentOptions, requestObject.proxy, secureLookup);
...
function convertN8nRequestToAxios(n8nRequest, ssrfBridge) {
    ...
    const secureLookup = ssrfBridge?.createSecureLookup();
    (0, utils_1.setAxiosAgents)(axiosRequest, agentOptions, proxy, secureLookup);
...
function resolveLegacyRequestUrl(requestObject) {
    ...
    return (0, utils_1.buildTargetUrl)(rawUrl, baseURL) ?? rawUrl;
}
