...
function tagMiddleware(middleware, apiKeyScope) {
...
const apiKeyHasScope = (apiKeyScope) => {
    const middleware = di_1.Container.get(public_api_key_service_1.PublicApiKeyService).getApiKeyScopeMiddleware(apiKeyScope);
...
const apiKeyHasScopeWithGlobalScopeFallback = (config) => {
    ...
    return tagMiddleware(di_1.Container.get(public_api_key_service_1.PublicApiKeyService).getApiKeyScopeMiddleware(scope), scope);
};
