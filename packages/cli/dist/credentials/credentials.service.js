...
let CredentialsService = class CredentialsService {
    ...
    redactValues(data, props, credentialType) {
        ...
        if (credentialType === 'httpCustomAuth' && 'json' in data) {
            data.json =
                data.json && String(data.json).length > 0
                    ? n8n_workflow_1.CREDENTIAL_BLANKING_VALUE
                    : n8n_workflow_1.CREDENTIAL_EMPTY_VALUE;
        }
        return data;
    }
    redactCollectionOption(data, option) {
        ...
        if (Array.isArray(values)) {
            for (let i = 0; i < values.length; i++) {
                values[i] = this.redactValues(values[i], option.values, undefined);
            }
        }
        else if (typeof values === 'object' && values !== null) {
            data[collectionValuesKey] = this.redactValues(values, option.values, undefined);
        }
    }
