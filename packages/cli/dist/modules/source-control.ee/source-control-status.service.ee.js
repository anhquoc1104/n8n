...
let SourceControlStatusService = class SourceControlStatusService {
    constructor(logger, gitService, sourceControlImportService, sourceControlPreferencesService, tagRepository, folderRepository, workflowRepository, eventService) {
        this.logger = logger;
    ...
    async getStatusWorkflows(options, context, sourceControlledFiles, collectVerbose) {
        const [wfRemoteVersionIds, wfLocalVersionIds, foldersMappingsRemote, foldersMappingsLocal] = await Promise.all([
        ...
        const remoteFoldersById = new Map(foldersMappingsRemote.folders.map((folder) => [
            folder.id,
